...
- `2026-09-12` - First working app built. Added AsyncStorage (native, so it needs a new APK,
  not an OTA update). New screens: first-run welcome (4 questions, once only), home list with
  search, shop detail with Call + Message + Directions, and Settings. Light and dark themes,
  English and Taglish, all persisted. `npx tsc --noEmit` clean. Route types had to be
  regenerated by starting the dev server once; typed routes are on, so adding a screen
  without running Metro produces four confusing type errors that are not in the code.

- `2026-09-12` - Build 2 submitted (AsyncStorage, so a new APK rather than an OTA update).
  `expo doctor` fails on it with 14 patch version mismatches: investigated, benign, caused by
  Expo publishing SDK 57 patches on 11 Sep after this project was scaffolded. Does not stop
  the build. Deferred to the step 6 build. Four recurring non-problems are now written up in
